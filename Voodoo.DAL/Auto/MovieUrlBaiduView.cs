...
namespace Voodoo.DAL
{
	...
	///<summary>
	///表MovieUrlBaidu的数据操作类
	///</summary>
    public partial class MovieUrlBaiduView
    {
		...
		public static void Insert(MovieUrlBaidu M)
        {
			...
			sb.Append("insert into [MovieUrlBaidu]([id],[MovieID],[MovieTitle],[Title],[UpdateTime],[Enable],[Url]) values(");
        ...
        /// </summary>
        /// <param name="Ms"></param>
        public static void Update(List<MovieUrlBaidu> Ms)
        {
            foreach (MovieUrlBaidu M in Ms)
            {
                Update(M);
            }
        }
        ...
        /// <returns></returns>
		public static MovieUrlBaidu GetModelByID(string id)
		{
			IDbHelper Sql = GetHelper();
			MovieUrlBaidu M = new MovieUrlBaidu();
			DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [id],[MovieID],[MovieTitle],[Title],[UpdateTime],[Enable],[Url] from [MovieUrlBaidu] where Id='" + id.ToString()+"'", true);
			if (!Rs.Read())
			{
                M.Id = long.MinValue;
			}
		...
		{
			IDbHelper Sql = GetHelper();
            return Sql.ExecuteDataTable(CommandType.Text, "select [id],[MovieID],[MovieTitle],[Title],[UpdateTime],[Enable],[Url] from [MovieUrlBaidu] where "+ m_where);
		}
        ...
        {   
            IDbHelper Sql = GetHelper();
            DataTable dt = Sql.ExecuteDataTable(CommandType.Text, "select top "+ top.ToString() +"  [id],[MovieID],[MovieTitle],[Title],[UpdateTime],[Enable],[Url] from [MovieUrlBaidu] where "+ m_where);
            return dt;
        }
		#endregion
		...
		{
			IDbHelper Sql = GetHelper();
			return Convert.ToInt32(Sql.ExecuteScalar(CommandType.Text,"select count(0) from [MovieUrlBaidu] where "+m_where));
		}
		#endregion
		...
		public static bool Exist(string m_where)
		{
            ...
            sd = Sql.ExecuteReader(CommandType.Text, "select 1 from [MovieUrlBaidu] where " + m_where, true);
            if (sd.Read())
            {
                returnValue = true;
            }
		...
		#region List<MovieUrlBaidu>获取符合条件记录的实体列表,慎用！！！！
		/// <summary>
        /// List<MovieUrlBaidu>获取符合条件记录的实体列表,慎用！！！！
        /// </summary>
        /// <param name="m_where">条件语句，不包含“where”</param>
        /// <returns></returns>
		public static List<MovieUrlBaidu> GetModelList(string m_where)
		{	
			return DataTableToList(getTable(m_where));
		}
		public static List<MovieUrlBaidu> GetModelList(string m_where,int top)
		{	
			return DataTableToList(getTable(m_where, top));
		}
		public static List<MovieUrlBaidu> GetModelList()
        {
		...
		public static bool Del(string m_where)
		{
			...
			try
			{
				Sql.ExecuteNonQuery(CommandType.Text, "delete from [MovieUrlBaidu] where "+ m_where);
				return true;
			}
			catch
			{
