...
namespace Voodoo.DAL
{
    ...
    public partial class SysKeywordView
    {
        ...
        public static void Insert(SysKeyword M)
        {
            ...
            sb.Append("insert into [SysKeyword]([Keyword],[ModelID]) values(");
            sb.Append("'" + M.Keyword + "'");
            sb.Append(",");
        ...
        public static SysKeyword GetModelByID(string id)
        {
            IDbHelper Sql = GetHelper();
            SysKeyword M = new SysKeyword();
            DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [ID],[Keyword],[ModelID] from [SysKeyword] where ID='" + id.ToString() + "'", true);
            if (!Rs.Read())
            {
                M.ID = 0;
            }
        ...
        public static SysKeyword Find(string m_where)
        {
            IDbHelper Sql = GetHelper();
            SysKeyword M = new SysKeyword();
            DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [ID],[Keyword],[ModelID] from [SysKeyword] where " + m_where, true);
            if (!Rs.Read())
            {
                M.ID = 0;
            }
        ...
        {
            IDbHelper Sql = GetHelper();
            return Sql.ExecuteDataTable(CommandType.Text, "select [ID],[Keyword],[ModelID] from [SysKeyword] where " + m_where);
        }
        ...
        {
            IDbHelper Sql = GetHelper();
            DataTable dt = Sql.ExecuteDataTable(CommandType.Text, "select top " + top.ToString() + "  [ID],[Keyword],[ModelID] from [SysKeyword] where " + m_where);
            return dt;
        }
        #endregion
