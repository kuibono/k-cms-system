...
namespace Voodoo.DAL
{
	...
	///<summary>
	///表TemplateReply的数据操作类
	///</summary>
    public partial class TemplateReplyView
    {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        protected static List<TemplateReply> DataTableToList(DataTable dt)
        {
            List<TemplateReply> Ms = new List<TemplateReply>();
			
			for (int i = 0; i < dt.Rows.Count; i++)
            {
				TemplateReply M = new TemplateReply();
        ...
        /// <returns></returns>
		public static void Insert(TemplateReply M)
        {
            IDbHelper Sql = GetHelper();
            StringBuilder sb = new StringBuilder();			
			
			sb.Append("insert into [TemplateReply]([GroupID],[Content]) values(");
			...
			{
				sb.Append(";select max(ID) from TemplateReply");	
			}
        ...
        /// <param name="M">赋值后的实体</param>
        /// <returns></returns>
		public static int Update(TemplateReply M)
        {
            IDbHelper Sql = GetHelper();
			StringBuilder sb = new StringBuilder();
            sb.Append("update [TemplateReply] set ");
        ...
        /// </summary>
        /// <param name="Ms"></param>
        public static void Update(List<TemplateReply> Ms)
        {
            foreach (TemplateReply M in Ms)
            {
                Update(M);
            }
        }
        ...
        /// <returns></returns>
		public static TemplateReply GetModelByID(string id)
		{
			IDbHelper Sql = GetHelper();
			TemplateReply M = new TemplateReply();
			DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [ID],[GroupID],[Content] from [TemplateReply] where ID='" + id.ToString()+"'", true);
			if (!Rs.Read())
			{
					M.ID=0;
			}
        ...
        /// <returns></returns>
		public static TemplateReply Find(string m_where)
		{
			IDbHelper Sql = GetHelper();
            TemplateReply M = new TemplateReply();
            DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [ID],[GroupID],[Content] from [TemplateReply] where " + m_where, true);
			if (!Rs.Read())
            {
					M.ID=0;
            }
		...
		{
			IDbHelper Sql = GetHelper();
            return Sql.ExecuteDataTable(CommandType.Text, "select [ID],[GroupID],[Content] from [TemplateReply] where "+ m_where);
		}
        ...
        {   
            IDbHelper Sql = GetHelper();
            DataTable dt = Sql.ExecuteDataTable(CommandType.Text, "select top "+ top.ToString() +"  [ID],[GroupID],[Content] from [TemplateReply] where "+ m_where);
            return dt;
        }
		#endregion
		...
		{
			IDbHelper Sql = GetHelper();
			return Convert.ToInt32(Sql.ExecuteScalar(CommandType.Text,"select count(0) from [TemplateReply] where "+m_where));
		}
		#endregion
		...
		public static bool Exist(string m_where)
		{
            ...
            sd = Sql.ExecuteReader(CommandType.Text, "select 1 from [TemplateReply] where " + m_where, true);
            if (sd.Read())
            {
                returnValue = true;
            }
		...
		#region List<TemplateReply>获取符合条件记录的实体列表,慎用！！！！
		/// <summary>
        /// List<TemplateReply>获取符合条件记录的实体列表,慎用！！！！
        /// </summary>
        /// <param name="m_where">条件语句，不包含“where”</param>
        /// <returns></returns>
		public static List<TemplateReply> GetModelList(string m_where)
		{	
			return DataTableToList(getTable(m_where));
		}
		public static List<TemplateReply> GetModelList(string m_where,int top)
		{	
			return DataTableToList(getTable(m_where, top));
		}
		public static List<TemplateReply> GetModelList()
        {
		...
		public static bool Del(string m_where)
		{
			...
			try
			{
				Sql.ExecuteNonQuery(CommandType.Text, "delete from [TemplateReply] where "+ m_where);
				return true;
			}
			catch
			{
