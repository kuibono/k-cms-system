...
namespace Voodoo.DAL
{
	...
	///表TemplateFace的数据操作类
	///</summary>
    public partial class TemplateFaceView
    {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        protected static List<TemplateFace> DataTableToList(DataTable dt)
        {
            List<TemplateFace> Ms = new List<TemplateFace>();
			
			for (int i = 0; i < dt.Rows.Count; i++)
            {
				TemplateFace M = new TemplateFace();
        ...
        /// <returns></returns>
		public static void Insert(TemplateFace M)
        {
            IDbHelper Sql = GetHelper();
            StringBuilder sb = new StringBuilder();			
			
			sb.Append("insert into [TemplateFace]([GroupID],[Content]) values(");
			...
			{
				sb.Append(";select max(ID) from TemplateFace");	
			}
        ...
        /// <param name="M">赋值后的实体</param>
        /// <returns></returns>
		public static int Update(TemplateFace M)
        {
            IDbHelper Sql = GetHelper();
			StringBuilder sb = new StringBuilder();
            sb.Append("update [TemplateFace] set ");
        ...
        /// </summary>
        /// <param name="Ms"></param>
        public static void Update(List<TemplateFace> Ms)
        {
            foreach (TemplateFace M in Ms)
            {
                Update(M);
            }
        }
        ...
        /// <returns></returns>
		public static TemplateFace GetModelByID(string id)
		{
			IDbHelper Sql = GetHelper();
			TemplateFace M = new TemplateFace();
			DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [ID],[GroupID],[Content] from [TemplateFace] where ID=" + id.ToString(), true);
			if (!Rs.Read())
			{
					M.ID=0;
			}
        ...
        /// <returns></returns>
		public static TemplateFace Find(string m_where)
		{
			IDbHelper Sql = GetHelper();
            TemplateFace M = new TemplateFace();
            DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [ID],[GroupID],[Content] from [TemplateFace] where " + m_where, true);
			if (!Rs.Read())
            {
					M.ID=0;
            }
		...
		{
			IDbHelper Sql = GetHelper();
            return Sql.ExecuteDataTable(CommandType.Text, "select [ID],[GroupID],[Content] from [TemplateFace] where "+ m_where);
		}
        ...
        {   
            IDbHelper Sql = GetHelper();
            DataTable dt = Sql.ExecuteDataTable(CommandType.Text, "select top "+ top.ToString() +"  [ID],[GroupID],[Content] from [TemplateFace] where "+ m_where);
            return dt;
        }
		#endregion
		...
		{
			IDbHelper Sql = GetHelper();
			return Convert.ToInt32(Sql.ExecuteScalar(CommandType.Text,"select count(0) from [TemplateFace] where "+m_where));
		}
		#endregion
		...
		public static bool Exist(string m_where)
		{
            ...
            sd = Sql.ExecuteReader(CommandType.Text, "select 1 from [TemplateFace] where " + m_where, true);
            if (sd.Read())
            {
                returnValue = true;
            }
		...
		#region List<TemplateFace>获取符合条件记录的实体列表,慎用！！！！
		/// <summary>
        /// List<TemplateFace>获取符合条件记录的实体列表,慎用！！！！
        /// </summary>
        /// <param name="m_where">条件语句，不包含“where”</param>
        /// <returns></returns>
		public static List<TemplateFace> GetModelList(string m_where)
		{	
			return DataTableToList(getTable(m_where));
		}
		public static List<TemplateFace> GetModelList(string m_where,int top)
		{	
			return DataTableToList(getTable(m_where, top));
		}
		public static List<TemplateFace> GetModelList()
        {
		...
		public static bool Del(string m_where)
		{
			...
			try
			{
				Sql.ExecuteNonQuery(CommandType.Text, "delete from [TemplateFace] where "+ m_where);
				return true;
			}
			catch
			{
