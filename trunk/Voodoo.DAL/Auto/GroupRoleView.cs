...
namespace Voodoo.DAL
{
	...
	///表GroupRole的数据操作类
	///</summary>
    public partial class GroupRoleView
    {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        protected static List<GroupRole> DataTableToList(DataTable dt)
        {
            List<GroupRole> Ms = new List<GroupRole>();
			...
			for (int i = 0; i < dt.Rows.Count; i++)
            {
				...
				M.RoleID=dt.Rows[i]["RoleID"].ToInt32();
        ...
        /// <param name="M">赋值后的实体</param>
        /// <returns></returns>
		public static int Update(GroupRole M)
        {
            IDbHelper Sql = GetHelper();
			StringBuilder sb = new StringBuilder();
            sb.Append("update [GroupRole] set ");
			
			sb.Append("[UserID]="+M.UserID.ToS());
			sb.Append(",");
			sb.Append("[RoleID]="+M.RoleID.ToS());
        ...
        /// </summary>
        /// <param name="Ms"></param>
        public static void Update(List<GroupRole> Ms)
        {
            foreach (GroupRole M in Ms)
            {
                Update(M);
            }
        }
        ...
        /// <returns></returns>
		public static GroupRole GetModelByID(string id)
		{
			IDbHelper Sql = GetHelper();
			GroupRole M = new GroupRole();
        ...
        /// <returns></returns>
		public static GroupRole Find(string m_where)
		{
			IDbHelper Sql = GetHelper();
            GroupRole M = new GroupRole();
            DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [ID],[UserID],[RoleID] from [GroupRole] where " + m_where, true);
			if (!Rs.Read())
            {
					M.ID=0;
            }
		...
		{
			IDbHelper Sql = GetHelper();
            return Sql.ExecuteDataTable(CommandType.Text, "select [ID],[UserID],[RoleID] from [GroupRole] where "+ m_where);
		}
        ...
        {   
            IDbHelper Sql = GetHelper();
            DataTable dt = Sql.ExecuteDataTable(CommandType.Text, "select top "+ top.ToString() +"  [ID],[UserID],[RoleID] from [GroupRole] where "+ m_where);
            return dt;
        }
		#endregion
		...
		{
			IDbHelper Sql = GetHelper();
			return Convert.ToInt32(Sql.ExecuteScalar(CommandType.Text,"select count(0) from [GroupRole] where "+m_where));
		}
		#endregion
		...
		public static bool Exist(string m_where)
		{
            ...
            sd = Sql.ExecuteReader(CommandType.Text, "select 1 from [GroupRole] where " + m_where, true);
            if (sd.Read())
            {
                returnValue = true;
            }
		...
		#region List<GroupRole>获取符合条件记录的实体列表,慎用！！！！
		/// <summary>
        /// List<GroupRole>获取符合条件记录的实体列表,慎用！！！！
        /// </summary>
        /// <param name="m_where">条件语句，不包含“where”</param>
        /// <returns></returns>
		public static List<GroupRole> GetModelList(string m_where)
		{	
			return DataTableToList(getTable(m_where));
		}
		public static List<GroupRole> GetModelList(string m_where,int top)
		{	
			return DataTableToList(getTable(m_where, top));
		}
		public static List<GroupRole> GetModelList()
        {
		...
		public static bool Del(string m_where)
		{
			...
			try
			{
				Sql.ExecuteNonQuery(CommandType.Text, "delete from [GroupRole] where "+ m_where);
				return true;
			}
			catch
			{
