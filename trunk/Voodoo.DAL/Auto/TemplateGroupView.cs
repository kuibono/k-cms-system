...
namespace Voodoo.DAL
{
	...
	///<summary>
	///表TemplateGroup的数据操作类
	///</summary>
    public partial class TemplateGroupView
    {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        protected static List<TemplateGroup> DataTableToList(DataTable dt)
        {
            List<TemplateGroup> Ms = new List<TemplateGroup>();
			...
			for (int i = 0; i < dt.Rows.Count; i++)
            {
				TemplateGroup M = new TemplateGroup();
					...
					M.IsDefault=dt.Rows[i]["IsDefault"].ToBoolean();
        ...
        /// <returns></returns>
		public static void Insert(TemplateGroup M)
        {
            IDbHelper Sql = GetHelper();
            StringBuilder sb = new StringBuilder();			
			
			sb.Append("insert into TemplateGroup(GroupName,IsDefault) values(");
			sb.Append("'"+M.GroupName+"'");
			sb.Append(",");	
			sb.Append(M.IsDefault.ToS());
			sb.Append(")");
			...
			{
				sb.Append(";select max(ID) from TemplateGroup");	
			}
        ...
        /// <param name="M">赋值后的实体</param>
        /// <returns></returns>
		public static int Update(TemplateGroup M)
        {
            IDbHelper Sql = GetHelper();
			StringBuilder sb = new StringBuilder();
            sb.Append("update TemplateGroup set ");
			
			sb.Append("GroupName='"+M.GroupName+"'");
			sb.Append(",");
			sb.Append("IsDefault="+M.IsDefault.ToS());
        ...
        /// </summary>
        /// <param name="Ms"></param>
        public static void Update(List<TemplateGroup> Ms)
        {
            foreach (TemplateGroup M in Ms)
            {
                Update(M);
            }
        }
        ...
        /// <returns></returns>
		public static TemplateGroup GetModelByID(string id)
		{
			IDbHelper Sql = GetHelper();
			TemplateGroup M = new TemplateGroup();
			...
			else
			{
				...
				M.IsDefault=Rs["IsDefault"].ToBoolean();
			}
			Rs.Close();
			Rs = null;
			return M;
        ...
        /// <returns></returns>
		public static TemplateGroup Find(string m_where)
		{
			IDbHelper Sql = GetHelper();
            TemplateGroup M = new TemplateGroup();
            DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select ID,GroupName,IsDefault from TemplateGroup where " + m_where, true);
			if (!Rs.Read())
            {
					M.ID=0;
            }
		...
		{
			IDbHelper Sql = GetHelper();
            return Sql.ExecuteDataTable(CommandType.Text, "select ID,GroupName,IsDefault from TemplateGroup where "+ m_where);
		}
        ...
        {   
            IDbHelper Sql = GetHelper();
            DataTable dt = Sql.ExecuteDataTable(CommandType.Text, "select top "+ top.ToString() +"  ID,GroupName,IsDefault from TemplateGroup where "+ m_where);
            return dt;
        }
		#endregion
		...
		{
			IDbHelper Sql = GetHelper();
			return Convert.ToInt32(Sql.ExecuteScalar(CommandType.Text,"select count(0) from TemplateGroup where "+m_where));
		}
		#endregion
		...
		public static bool Exist(string m_where)
		{
            ...
            sd = Sql.ExecuteReader(CommandType.Text, "select 1 from TemplateGroup where " + m_where, true);
            if (sd.Read())
            {
                returnValue = true;
            }
		...
		#region List<TemplateGroup>获取符合条件记录的实体列表,慎用！！！！
		/// <summary>
        /// List<TemplateGroup>获取符合条件记录的实体列表,慎用！！！！
        /// </summary>
        /// <param name="m_where">条件语句，不包含“where”</param>
        /// <returns></returns>
		public static List<TemplateGroup> GetModelList(string m_where)
		{	
			return DataTableToList(getTable(m_where));
		}
		public static List<TemplateGroup> GetModelList(string m_where,int top)
		{	
			return DataTableToList(getTable(m_where, top));
		}
		public static List<TemplateGroup> GetModelList()
        {
		...
		public static bool Del(string m_where)
		{
			...
			try
			{
				Sql.ExecuteNonQuery(CommandType.Text, "delete from TemplateGroup where "+ m_where);
				return true;
			}
			catch
			{
