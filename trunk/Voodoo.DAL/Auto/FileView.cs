...
namespace Voodoo.DAL
{
    ...
    public partial class FileView
    {
		...
		public static void Insert(File M)
        {
			...
			sb.Append("insert into [File]([ClassID],[UpTime],[FileType],[FileSize],[FileDirectory],[FileName],[FileExtName],[FilePath],[SmallPath]) values(");
			sb.Append(M.ClassID.ToS());
			sb.Append(",");	
			sb.Append("N'"+M.UpTime+"'");
			sb.Append(",");	
			sb.Append(M.FileType.ToS());
			sb.Append(",");	
			sb.Append(M.FileSize.ToS());
			sb.Append(",");	
			sb.Append("N'"+M.FileDirectory+"'");
			sb.Append(",");	
			sb.Append("N'"+M.FileName+"'");
			sb.Append(",");	
			sb.Append("N'"+M.FileExtName+"'");
			sb.Append(",");	
		...
		public static int Update(File M)
        {
			...
			sb.Append(",");
			sb.Append("[UpTime]=N'"+M.UpTime+"'");
			sb.Append(",");
			sb.Append("[FileType]="+M.FileType.ToS());
			sb.Append(",");
			sb.Append("[FileSize]="+M.FileSize.ToS());
			sb.Append(",");
			sb.Append("[FileDirectory]=N'"+M.FileDirectory+"'");
			sb.Append(",");
			sb.Append("[FileName]=N'"+M.FileName+"'");
			sb.Append(",");
			sb.Append("[FileExtName]=N'"+M.FileExtName+"'");
			sb.Append(",");
		...
		public static File Find(string m_where)
		{
			IDbHelper Sql = GetHelper();
            File M = new File();
            DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [ID],[ClassID],[UpTime],[FileType],[FileSize],[FileDirectory],[FileName],[FileExtName],[FilePath],[SmallPath] from [File] where " + m_where, true);
			if (!Rs.Read())
            {
					M.ID=0;
            }
		...
		{
			IDbHelper Sql = GetHelper();
            return Sql.ExecuteDataTable(CommandType.Text, "select [ID],[ClassID],[UpTime],[FileType],[FileSize],[FileDirectory],[FileName],[FileExtName],[FilePath],[SmallPath] from [File] where "+ m_where);
		}
        ...
        {   
            IDbHelper Sql = GetHelper();
            DataTable dt = Sql.ExecuteDataTable(CommandType.Text, "select top "+ top.ToString() +"  [ID],[ClassID],[UpTime],[FileType],[FileSize],[FileDirectory],[FileName],[FileExtName],[FilePath],[SmallPath] from [File] where "+ m_where);
            return dt;
        }
		#endregion
