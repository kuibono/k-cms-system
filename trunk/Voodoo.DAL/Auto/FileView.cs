...
namespace Voodoo.DAL
{
    ...
    public partial class FileView
    {
		...
		public static void Insert(File M)
        {
			...
			sb.Append("insert into File(UpTime,FileType,FileSize,FileDirectory,FileName,FileExtName) values(");
			sb.Append("'"+M.UpTime+"'");
			sb.Append(",");	
		...
		public static File GetModelByID(string id)
		{
			IDbHelper Sql = GetHelper();
			File M = new File();
			DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select ID,UpTime,FileType,FileSize,FileDirectory,FileName,FileExtName from File where ID='" + id.ToString()+"'", true);
			if (!Rs.Read())
			{
					M.ID=0;
			}
		...
		public static File Find(string m_where)
		{
			IDbHelper Sql = GetHelper();
            File M = new File();
            DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select ID,UpTime,FileType,FileSize,FileDirectory,FileName,FileExtName from File where " + m_where, true);
			if (!Rs.Read())
            {
					M.ID=0;
            }
