...
namespace Voodoo.DAL
{
    ...
    public partial class TemplateListView
    {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        protected static List<TemplateList> DataTableToList(DataTable dt)
        {
            List<TemplateList> Ms = new List<TemplateList>();
        ...
        /// <returns></returns>
		public static void Insert(TemplateList M)
        {
            IDbHelper Sql = GetHelper();
            StringBuilder sb = new StringBuilder();			
			
			sb.Append("insert into TemplateList(GroupID,TempName,SysModel,CutKeywords,CutTitle,ShowRecordCount,TimeFormat,Content,ListVar) values(");
			...
			{
				sb.Append(";select max(ID) from TemplateList");	
			}
        ...
        /// <param name="M">赋值后的实体</param>
        /// <returns></returns>
		public static int Update(TemplateList M)
        {
            IDbHelper Sql = GetHelper();
			StringBuilder sb = new StringBuilder();
            sb.Append("update TemplateList set ");
        ...
        /// </summary>
        /// <param name="Ms"></param>
        public static void Update(List<TemplateList> Ms)
        {
            foreach (TemplateList M in Ms)
            {
                Update(M);
            }
        }
        ...
        /// <returns></returns>
		public static TemplateList GetModelByID(string id)
		{
			IDbHelper Sql = GetHelper();
			TemplateList M = new TemplateList();
			DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select ID,GroupID,TempName,SysModel,CutKeywords,CutTitle,ShowRecordCount,TimeFormat,Content,ListVar from TemplateList where ID='" + id.ToString()+"'", true);
			if (!Rs.Read())
			{
					M.ID=0;
			}
		...
		{
			IDbHelper Sql = GetHelper();
            return Sql.ExecuteDataTable(CommandType.Text, "select ID,GroupID,TempName,SysModel,CutKeywords,CutTitle,ShowRecordCount,TimeFormat,Content,ListVar from TemplateList where "+ m_where);
		}
        ...
        {   
            IDbHelper Sql = GetHelper();
            DataTable dt = Sql.ExecuteDataTable(CommandType.Text, "select top "+ top.ToString() +"  ID,GroupID,TempName,SysModel,CutKeywords,CutTitle,ShowRecordCount,TimeFormat,Content,ListVar from TemplateList where "+ m_where);
            return dt;
        }
		#endregion
		...
		{
			IDbHelper Sql = GetHelper();
			return Convert.ToInt32(Sql.ExecuteScalar(CommandType.Text,"select count(0) from TemplateList where "+m_where));
		}
		#endregion
		...
		public static bool Exist(string m_where)
		{
            ...
            sd = Sql.ExecuteReader(CommandType.Text, "select 1 from TemplateList where " + m_where, true);
            if (sd.Read())
            {
                returnValue = true;
            }
		...
		#region List<TemplateList>获取符合条件记录的实体列表,慎用！！！！
		/// <summary>
        /// List<TemplateList>获取符合条件记录的实体列表,慎用！！！！
        /// </summary>
        /// <param name="m_where">条件语句，不包含“where”</param>
        /// <returns></returns>
		public static List<TemplateList> GetModelList(string m_where)
		{	
			return DataTableToList(getTable(m_where));
		}
		public static List<TemplateList> GetModelList(string m_where,int top)
		{	
			return DataTableToList(getTable(m_where, top));
		}
		public static List<TemplateList> GetModelList()
        {
		...
		public static bool Del(string m_where)
		{
			...
			try
			{
				Sql.ExecuteNonQuery(CommandType.Text, "delete from TemplateList where "+ m_where);
				return true;
			}
			catch
			{
