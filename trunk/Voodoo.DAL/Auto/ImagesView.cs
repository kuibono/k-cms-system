...
namespace Voodoo.DAL
{
    ...
    public partial class ImagesView
    {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        protected static List<Images> DataTableToList(DataTable dt)
        {
            List<Images> Ms = new List<Images>();
			
			for (int i = 0; i < dt.Rows.Count; i++)
            {
				Images M = new Images();
				M.ID=dt.Rows[i]["ID"].ToInt32();
				M.AlbumID=dt.Rows[i]["AlbumID"].ToInt32();
				M.Title=dt.Rows[i]["Title"].ToString();
				M.Intro=dt.Rows[i]["Intro"].ToString();
				M.UploadTime=dt.Rows[i]["UploadTime"].ToDateTime();
				...
				M.FileSize=dt.Rows[i]["FileSize"].ToInt32();
        ...
        /// <returns></returns>
		public static void Insert(Images M)
        {
            IDbHelper Sql = GetHelper();
            StringBuilder sb = new StringBuilder();			
			
			sb.Append("insert into [Images]([AlbumID],[Title],[Intro],[UploadTime],[ClickCount],[ReplyCount],[FilePath],[SmallPath],[FileSize],[ExtName]) values(");
			sb.Append(M.AlbumID.ToS());
			sb.Append(",");	
			sb.Append("N'"+M.Title+"'");
			sb.Append(",");	
			sb.Append("N'"+M.Intro+"'");
			sb.Append(",");	
			sb.Append("N'"+M.UploadTime+"'");
			sb.Append(",");	
			sb.Append(M.ClickCount.ToS());
			sb.Append(",");	
			sb.Append(M.ReplyCount.ToS());
			sb.Append(",");	
			sb.Append("N'"+M.FilePath+"'");
			sb.Append(",");	
			sb.Append("N'"+M.SmallPath+"'");
			sb.Append(",");	
			...
			{
				sb.Append(";select max(ID) from Images");	
			}
        ...
        /// <param name="M">赋值后的实体</param>
        /// <returns></returns>
		public static int Update(Images M)
        {
            IDbHelper Sql = GetHelper();
			StringBuilder sb = new StringBuilder();
            sb.Append("update [Images] set ");
			
			sb.Append("[AlbumID]="+M.AlbumID.ToS());
			sb.Append(",");
			sb.Append("[Title]=N'"+M.Title+"'");
			sb.Append(",");
			sb.Append("[Intro]=N'"+M.Intro+"'");
			sb.Append(",");
			sb.Append("[UploadTime]=N'"+M.UploadTime+"'");
			sb.Append(",");
			sb.Append("[ClickCount]="+M.ClickCount.ToS());
			sb.Append(",");
			sb.Append("[ReplyCount]="+M.ReplyCount.ToS());
			sb.Append(",");
			sb.Append("[FilePath]=N'"+M.FilePath+"'");
			sb.Append(",");
			sb.Append("[SmallPath]=N'"+M.SmallPath+"'");
			sb.Append(",");
        ...
        /// </summary>
        /// <param name="Ms"></param>
        public static void Update(List<Images> Ms)
        {
            foreach (Images M in Ms)
            {
                Update(M);
            }
        }
        ...
        /// <returns></returns>
		public static Images GetModelByID(string id)
		{
			IDbHelper Sql = GetHelper();
			Images M = new Images();
			DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [ID],[AlbumID],[Title],[Intro],[UploadTime],[ClickCount],[ReplyCount],[FilePath],[SmallPath],[FileSize],[ExtName] from [Images] where ID='" + id.ToString()+"'", true);
			if (!Rs.Read())
			{
					M.ID=0;
			}
			...
			{
				M.ID=Rs["ID"].ToInt32();
				M.AlbumID=Rs["AlbumID"].ToInt32();
				M.Title=Rs["Title"].ToString();
				M.Intro=Rs["Intro"].ToString();
				M.UploadTime=Rs["UploadTime"].ToDateTime();
				...
				M.FileSize=Rs["FileSize"].ToInt32();
        ...
        /// <returns></returns>
		public static Images Find(string m_where)
		{
			IDbHelper Sql = GetHelper();
            Images M = new Images();
            DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [ID],[AlbumID],[Title],[Intro],[UploadTime],[ClickCount],[ReplyCount],[FilePath],[SmallPath],[FileSize],[ExtName] from [Images] where " + m_where, true);
			if (!Rs.Read())
            {
					M.ID=0;
            }
		...
		{
			IDbHelper Sql = GetHelper();
            return Sql.ExecuteDataTable(CommandType.Text, "select [ID],[AlbumID],[Title],[Intro],[UploadTime],[ClickCount],[ReplyCount],[FilePath],[SmallPath],[FileSize],[ExtName] from [Images] where "+ m_where);
		}
        ...
        {   
            IDbHelper Sql = GetHelper();
            DataTable dt = Sql.ExecuteDataTable(CommandType.Text, "select top "+ top.ToString() +"  [ID],[AlbumID],[Title],[Intro],[UploadTime],[ClickCount],[ReplyCount],[FilePath],[SmallPath],[FileSize],[ExtName] from [Images] where "+ m_where);
            return dt;
        }
		#endregion
		...
		{
			IDbHelper Sql = GetHelper();
			return Convert.ToInt32(Sql.ExecuteScalar(CommandType.Text,"select count(0) from [Images] where "+m_where));
		}
		#endregion
		...
		public static bool Exist(string m_where)
		{
            ...
            sd = Sql.ExecuteReader(CommandType.Text, "select 1 from [Images] where " + m_where, true);
            if (sd.Read())
            {
                returnValue = true;
            }
		...
		#region List<Images>获取符合条件记录的实体列表,慎用！！！！
		/// <summary>
        /// List<Images>获取符合条件记录的实体列表,慎用！！！！
        /// </summary>
        /// <param name="m_where">条件语句，不包含“where”</param>
        /// <returns></returns>
		public static List<Images> GetModelList(string m_where)
		{	
			return DataTableToList(getTable(m_where));
		}
		public static List<Images> GetModelList(string m_where,int top)
		{	
			return DataTableToList(getTable(m_where, top));
		}
		public static List<Images> GetModelList()
        {
		...
		public static bool Del(string m_where)
		{
			...
			try
			{
				Sql.ExecuteNonQuery(CommandType.Text, "delete from [Images] where "+ m_where);
				return true;
			}
			catch
			{
