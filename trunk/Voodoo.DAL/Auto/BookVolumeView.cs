...
namespace Voodoo.DAL
{
	...
	///表BookVolume的数据操作类
	///</summary>
    public partial class BookVolumeView
    {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        protected static List<BookVolume> DataTableToList(DataTable dt)
        {
            List<BookVolume> Ms = new List<BookVolume>();
			...
			for (int i = 0; i < dt.Rows.Count; i++)
            {
				...
				M.Booknclass=dt.Rows[i]["booknclass"].ToString();
        ...
        /// <returns></returns>
		public static void Insert(BookVolume M)
        {
            IDbHelper Sql = GetHelper();
            StringBuilder sb = new StringBuilder();			
			
			sb.Append("insert into [BookVolume]([ID],[BookID],[booktitle],[booknclass]) values(");
			sb.Append(M.BookID.ToS());
			sb.Append(",");	
			sb.Append("N'"+M.Booktitle+"'");
			sb.Append(",");	
			sb.Append("N'"+M.Booknclass+"'");
			sb.Append(")");
			...
			{
				sb.Append(";select max(ID) from BookVolume");	
			}
        ...
        /// <param name="M">赋值后的实体</param>
        /// <returns></returns>
		public static int Update(BookVolume M)
        {
            IDbHelper Sql = GetHelper();
			StringBuilder sb = new StringBuilder();
            sb.Append("update [BookVolume] set ");
			
			sb.Append("[BookID]="+M.BookID.ToS());
			sb.Append(",");
			sb.Append("[booktitle]=N'"+M.Booktitle+"'");
			sb.Append(",");
			sb.Append("[booknclass]=N'"+M.Booknclass+"'");
        ...
        /// </summary>
        /// <param name="Ms"></param>
        public static void Update(List<BookVolume> Ms)
        {
            foreach (BookVolume M in Ms)
            {
                Update(M);
            }
        }
        ...
        /// <returns></returns>
		public static BookVolume GetModelByID(string id)
		{
			IDbHelper Sql = GetHelper();
			BookVolume M = new BookVolume();
			...
			else
			{
				...
				M.Booknclass=Rs["booknclass"].ToString();
			}
			Rs.Close();
			Rs = null;
			return M;
        ...
        /// <returns></returns>
		public static BookVolume Find(string m_where)
		{
			IDbHelper Sql = GetHelper();
            BookVolume M = new BookVolume();
            DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [ID],[BookID],[booktitle],[booknclass] from [BookVolume] where " + m_where, true);
			if (!Rs.Read())
            {
					M.ID=long.MinValue;
            }
		...
		{
			IDbHelper Sql = GetHelper();
            return Sql.ExecuteDataTable(CommandType.Text, "select [ID],[BookID],[booktitle],[booknclass] from [BookVolume] where "+ m_where);
		}
        ...
        {   
            IDbHelper Sql = GetHelper();
            DataTable dt = Sql.ExecuteDataTable(CommandType.Text, "select top "+ top.ToString() +"  [ID],[BookID],[booktitle],[booknclass] from [BookVolume] where "+ m_where);
            return dt;
        }
		#endregion
		...
		{
			IDbHelper Sql = GetHelper();
			return Convert.ToInt32(Sql.ExecuteScalar(CommandType.Text,"select count(0) from [BookVolume] where "+m_where));
		}
		#endregion
		...
		public static bool Exist(string m_where)
		{
            ...
            sd = Sql.ExecuteReader(CommandType.Text, "select 1 from [BookVolume] where " + m_where, true);
            if (sd.Read())
            {
                returnValue = true;
            }
		...
		#region List<BookVolume>获取符合条件记录的实体列表,慎用！！！！
		/// <summary>
        /// List<BookVolume>获取符合条件记录的实体列表,慎用！！！！
        /// </summary>
        /// <param name="m_where">条件语句，不包含“where”</param>
        /// <returns></returns>
		public static List<BookVolume> GetModelList(string m_where)
		{	
			return DataTableToList(getTable(m_where));
		}
		public static List<BookVolume> GetModelList(string m_where,int top)
		{	
			return DataTableToList(getTable(m_where, top));
		}
		public static List<BookVolume> GetModelList()
        {
		...
		public static bool Del(string m_where)
		{
			...
			try
			{
				Sql.ExecuteNonQuery(CommandType.Text, "delete from [BookVolume] where "+ m_where);
				return true;
			}
			catch
			{
