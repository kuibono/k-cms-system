...
namespace Voodoo.DAL
{
	...
	///<summary>
	///表MovieUrlKuaib的数据操作类
	///</summary>
    public partial class MovieUrlKuaibView
    {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        protected static List<MovieUrlKuaib> DataTableToList(DataTable dt)
        {
            List<MovieUrlKuaib> Ms = new List<MovieUrlKuaib>();
        ...
        /// <returns></returns>
		public static void Insert(MovieUrlKuaib M)
        {
			...
			{
				sb.Append(";select max(id) from MovieUrlKuaib");	
			}
        ...
        /// <param name="M">赋值后的实体</param>
        /// <returns></returns>
		public static int Update(MovieUrlKuaib M)
        {
            IDbHelper Sql = GetHelper();
			StringBuilder sb = new StringBuilder();
            sb.Append("update [MovieUrlKuaib] set ");
        ...
        /// </summary>
        /// <param name="Ms"></param>
        public static void Update(List<MovieUrlKuaib> Ms)
        {
            foreach (MovieUrlKuaib M in Ms)
            {
                Update(M);
            }
        }
        ...
        /// <returns></returns>
		public static MovieUrlKuaib GetModelByID(string id)
		{
			IDbHelper Sql = GetHelper();
			MovieUrlKuaib M = new MovieUrlKuaib();
			DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [id],[MovieID],[MovieTitle],[Title],[UpdateTime],[Enable],[Url] from [MovieUrlKuaib] where Id='" + id.ToString()+"'", true);
			if (!Rs.Read())
			{
                M.Id = long.MinValue;
			}
        ...
        /// <returns></returns>
		public static MovieUrlKuaib Find(string m_where)
		{
			IDbHelper Sql = GetHelper();
            MovieUrlKuaib M = new MovieUrlKuaib();
            DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [id],[MovieID],[MovieTitle],[Title],[UpdateTime],[Enable],[Url] from [MovieUrlKuaib] where " + m_where, true);
			if (!Rs.Read())
            {
                M.Id = long.MinValue;
            }
		...
		{
			IDbHelper Sql = GetHelper();
            return Sql.ExecuteDataTable(CommandType.Text, "select [id],[MovieID],[MovieTitle],[Title],[UpdateTime],[Enable],[Url] from [MovieUrlKuaib] where "+ m_where);
		}
        ...
        {   
            IDbHelper Sql = GetHelper();
            DataTable dt = Sql.ExecuteDataTable(CommandType.Text, "select top "+ top.ToString() +"  [id],[MovieID],[MovieTitle],[Title],[UpdateTime],[Enable],[Url] from [MovieUrlKuaib] where "+ m_where);
            return dt;
        }
		#endregion
		...
		{
			IDbHelper Sql = GetHelper();
			return Convert.ToInt32(Sql.ExecuteScalar(CommandType.Text,"select count(0) from [MovieUrlKuaib] where "+m_where));
		}
		#endregion
		...
		public static bool Exist(string m_where)
		{
            ...
            sd = Sql.ExecuteReader(CommandType.Text, "select 1 from [MovieUrlKuaib] where " + m_where, true);
            if (sd.Read())
            {
                returnValue = true;
            }
		...
		#region List<MovieUrlKuaib>获取符合条件记录的实体列表,慎用！！！！
		/// <summary>
        /// List<MovieUrlKuaib>获取符合条件记录的实体列表,慎用！！！！
        /// </summary>
        /// <param name="m_where">条件语句，不包含“where”</param>
        /// <returns></returns>
		public static List<MovieUrlKuaib> GetModelList(string m_where)
		{	
			return DataTableToList(getTable(m_where));
		}
		public static List<MovieUrlKuaib> GetModelList(string m_where,int top)
		{	
			return DataTableToList(getTable(m_where, top));
		}
		public static List<MovieUrlKuaib> GetModelList()
        {
		...
		public static bool Del(string m_where)
		{
			...
			try
			{
				Sql.ExecuteNonQuery(CommandType.Text, "delete from [MovieUrlKuaib] where "+ m_where);
				return true;
			}
			catch
			{
