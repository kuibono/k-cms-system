...
namespace Voodoo.DAL
{
    ...
    public partial class LinkView
    {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        protected static List<Link> DataTableToList(DataTable dt)
        {
            List<Link> Ms = new List<Link>();

            for (int i = 0; i < dt.Rows.Count; i++)
            {
                Link M = new Link();
                M.ID = dt.Rows[i]["ID"].ToInt32();
                M.LinkTitle = dt.Rows[i]["LinkTitle"].ToString();
                M.Url = dt.Rows[i]["Url"].ToString();
                M.Index = dt.Rows[i]["Index"].ToInt32();
        ...
        /// <returns></returns>
        public static void Insert(Link M)
        {
            IDbHelper Sql = GetHelper();
            StringBuilder sb = new StringBuilder();

            sb.Append("insert into [Link]([LinkTitle],[Url],[Index]) values(");
            sb.Append("'" + M.LinkTitle + "'");
            sb.Append(",");
            sb.Append("'" + M.Url + "'");
            sb.Append(",");
            sb.Append(M.Index.ToS());
            sb.Append(")");
            ...
            {
                sb.Append(";select max(ID) from Link");
            }
        ...
        /// <param name="M">赋值后的实体</param>
        /// <returns></returns>
        public static int Update(Link M)
        {
            IDbHelper Sql = GetHelper();
            StringBuilder sb = new StringBuilder();
            sb.Append("update [Link] set ");

            sb.Append("[LinkTitle]='" + M.LinkTitle + "'");
            sb.Append(",");
            sb.Append("[Url]='" + M.Url + "'");
            sb.Append(",");
            sb.Append("[Index]=" + M.Index.ToS());
        ...
        /// </summary>
        /// <param name="Ms"></param>
        public static void Update(List<Link> Ms)
        {
            foreach (Link M in Ms)
            {
                Update(M);
            }
        }
        ...
        /// <returns></returns>
        public static Link GetModelByID(string id)
        {
            IDbHelper Sql = GetHelper();
            Link M = new Link();
            DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [ID],[LinkTitle],[Url],[Index] from [Link] where ID='" + id.ToString() + "'", true);
            if (!Rs.Read())
            {
                M.ID = 0;
            }
            ...
            {
                M.ID = Rs["ID"].ToInt32();
                M.LinkTitle = Rs["LinkTitle"].ToString();
                M.Url = Rs["Url"].ToString();
                M.Index = Rs["Index"].ToInt32();
            }
            Rs.Close();
            Rs = null;
            return M;
        ...
        /// <returns></returns>
        public static Link Find(string m_where)
        {
            IDbHelper Sql = GetHelper();
            Link M = new Link();
            DbDataReader Rs = Sql.ExecuteReader(CommandType.Text, "select [ID],[LinkTitle],[Url],[Index] from [Link] where " + m_where, true);
            if (!Rs.Read())
            {
                M.ID = 0;
            }
        ...
        {
            IDbHelper Sql = GetHelper();
            return Sql.ExecuteDataTable(CommandType.Text, "select [ID],[LinkTitle],[Url],[Index] from [Link] where " + m_where);
        }
        ...
        {
            IDbHelper Sql = GetHelper();
            DataTable dt = Sql.ExecuteDataTable(CommandType.Text, "select top " + top.ToString() + "  [ID],[LinkTitle],[Url],[Index] from [Link] where " + m_where);
            return dt;
        }
        #endregion
        ...
        {
            IDbHelper Sql = GetHelper();
            return Convert.ToInt32(Sql.ExecuteScalar(CommandType.Text, "select count(0) from [Link] where " + m_where));
        }
        #endregion
        ...
        public static bool Exist(string m_where)
        {
            ...
            sd = Sql.ExecuteReader(CommandType.Text, "select 1 from [Link] where " + m_where, true);
            if (sd.Read())
            {
                returnValue = true;
            }
        ...
        #region List<Link>获取符合条件记录的实体列表,慎用！！！！
        /// <summary>
        /// List<Link>获取符合条件记录的实体列表,慎用！！！！
        /// </summary>
        /// <param name="m_where">条件语句，不包含“where”</param>
        /// <returns></returns>
        public static List<Link> GetModelList(string m_where)
        {
            return DataTableToList(getTable(m_where));
        }
        public static List<Link> GetModelList(string m_where, int top)
        {
            return DataTableToList(getTable(m_where, top));
        }
        public static List<Link> GetModelList()
        {
        ...
        public static bool Del(string m_where)
        {
            ...
            try
            {
                Sql.ExecuteNonQuery(CommandType.Text, "delete from [Link] where " + m_where);
                return true;
            }
            catch
            {
